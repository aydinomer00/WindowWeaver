...
class WindowManager: ObservableObject {
    ...
    func repositionWindowUsingAppleScript(for app: NSRunningApplication, position: ScreenPosition.Third) {
        ...
        tell application "\(appName)"
            try
                set bounds of front window to {\(Int(newFrame.origin.x)), \(Int(newFrame.origin.y)), \(Int(newFrame
                        .origin.x + newFrame.size.width)), \(Int(newFrame.origin.y + newFrame.size.height))}
            on error errMsg
                return errMsg
            end try
        end tell
    ...
    private func checkAccessibilityPermissions() {
        ...
        alert
            .informativeText =
            "This application needs accessibility permission to work properly. Please grant permission in System Settings > Privacy & Security > Accessibility."
    ...
    private func updateActiveWindow() {
        ...
        print(
            "[DEBUG] updateActiveWindow: Selected app: \(app.localizedName ?? "Unknown") (PID: \(app.processIdentifier))"
        )
